...
**Note:** go-fuzz has recently added preliminary support for fuzzing [Go Modules](https://github.com/golang/go/wiki/Modules).  See the [section below](https://github.com/TomerYakir/go-fuzz/blob/master/README.md#modules-support) for more details. 
...
The [go-fuzz-corpus repository](https://github.com/TomerYakir/go-fuzz-corpus) contains 
a bunch of examples of test functions and initial input corpuses for various packages.

The next step is to get go-fuzz:

```
$ go get -u github.com/TomerYakir/go-fuzz/go-fuzz@latest github.com/TomerYakir/go-fuzz/go-fuzz-build@latest
```

Then, download the corpus and build the test program with necessary instrumentation:
```
$ go get -d github.com/TomerYakir/go-fuzz-corpus
$ cd $GOPATH/src/github.com/TomerYakir/go-fuzz-corpus
$ cd png
$ go-fuzz-build
```
...
go-fuzz-build will add a `require` for `github.com/TomerYakir/go-fuzz` to your go.mod. If desired, you may remove this once the build is complete.
...
```
$ cd $GOPATH/src/github.com/TomerYakir/go-fuzz-corpus/fmt
...
[#88](https://github.com/TomerYakir/go-fuzz/issues/88),
[#114](https://github.com/TomerYakir/go-fuzz/issues/114) and
https://github.com/TomerYakir/go-fuzz-corpus). Unfortunately, that means that
`go get -u` command will fail if you had a previous version installed.
Please remove $GOPATH/github.com/TomerYakir/go-fuzz before running `go get` again.
